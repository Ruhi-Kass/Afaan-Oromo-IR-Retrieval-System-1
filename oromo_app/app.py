  ...
  <div id="panel-corpus" class="panel">
    ...
    <div class="card">
      ...
      <div class="add-form">
        ...
        <div>
          <button class="btn btn-primary" onclick="addDocument()">➕ Add Text Document</button>
        </div>
      </div>
      <div id="add-result"></div>
      
      <hr style="margin: 24px 0;">
      <div class="card-title" style="margin-bottom:12px;">Upload Document File (.txt, .pdf, .docx)</div>
      <div class="add-form">
        <input type="file" id="upload-file" accept=".txt,.pdf,.docx" style="color:var(--text); font-size:14px; background:var(--surface2); padding:10px; border-radius:9px; border:1px solid var(--border);">
        <div>
          <button class="btn btn-secondary" onclick="uploadDocument()">📤 Upload & Index</button>
        </div>
      </div>
...
async function uploadDocument() {
  ...
  if (!fileInput.files || fileInput.files.length === 0) {
    document.getElementById('upload-result').innerHTML = '<div class="alert alert-error" style="margin-top:12px;">Please select a file to upload.</div>';
    return;
  }
  ...
  el.innerHTML = '<div class="alert alert-info" style="margin-top:12px;"><span class="spinner"></span> Uploading and processing...</div>';
  ...
  try {
    ...
    if (res.success) {
      el.innerHTML = `<div class="alert alert-success" style="margin-top:12px;">✓ File "${res.filename}" uploaded and indexed! Corpus reindexed.</div>`;
      fileInput.value = '';
      loadCorpus();
    } else {
      el.innerHTML = `<div class="alert alert-error" style="margin-top:12px;">Error: ${res.error || 'Failed to upload document.'}</div>`;
    }
  } catch (err) {
    el.innerHTML = `<div class="alert alert-error" style="margin-top:12px;">Upload failed: ${err.message}</div>`;
  }
}
...
def api_add_document():
    ...
    new_id = max((d['id'] for d in ir_system.documents), default=0) + 1
...
def api_upload_document():
    ...
    try:
        ...
        if not content.strip():
            return jsonify({'success': False, 'error': 'Could not extract any text from the file.'})
